...
class ACFNetwork(nn.Module):
    ...
    def __init__(self, arch, pretrained, num_classes, input_mode, acf_head='endpoints',input_channel=4,
        ...
        previous_first_layer = self.backbone.body.conv1
        self.backbone.body.conv1 = nn.Conv2d(input_channel, previous_first_layer.out_channels, previous_first_layer.kernel_size,
                                     stride=previous_first_layer.stride, bias=False)
        if input_channel == 4:
            ...
            self.backbone.body.conv1.weight.data[:, :3] = previous_first_layer.weight.data
            self.backbone.body.conv1.weight.data[:, 3] = previous_first_layer.weight.data.mean(dim=1, keepdim=False)
        elif input_channel==6:
            # change first layer to 4 channel for early fusion with 3 channel depth normal, load pretrained weights on RGB channels
            self.backbone.body.conv1.weight.data = previous_first_layer.weight.data.repeat(1, 2, 1, 1)
