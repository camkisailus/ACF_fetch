...
from keypoint_3d.kpoint_3d_branch import DenseGlobalFeatures, Vote_Kpoints_head, Vote_Kpoints_Predictor, vote_keypoint_inference
import config
...
class RoIHeadsExtend(nn.Module):
    def __init__(self, out_channels, num_classes, input_mode, acf_head,
        ...
        if self.input_mode == config.INPUT_RGBD:
            # for module in [self.box_roi_pool, self.box_head, self.box_predictor, self.shared_roi_pool, self.mask_head,
            #                self.mask_predictor]:
            #     for param in module.parameters():
            #         param.requires_grad = False

            # self.shared_roi_pool_depth = MultiScaleRoIAlign(
            #     featmap_names=[0, 1, 2, 3],
            #     output_size=14,
            #     sampling_ratio=2)
            # resolution = self.shared_roi_pool_depth.output_size[0]

            self.dense_global = DenseGlobalFeatures(kernel_size=(14*14))
            self.global_feature_dim = 1792
            ...
            if self.with_paf_branch:
                ...
                self.paf_predictor = MaskRCNNPredictor(mask_predictor_in_channels, mask_dim_reduced,
                                                       2 * (num_classes - 1))

            keypoint_layers = (1024, 1024, 512, 512, 512, 512, 512, 512)
            self.keypoint_dim_reduced = keypoint_layers[-1]
            if self.with_3d_keypoints:
                self.vote_keypoint_head = Vote_Kpoints_head(self.global_feature_dim, keypoint_layers)
                self.vote_keypoint_predictor = Vote_Kpoints_Predictor(self.keypoint_dim_reduced, 3*(num_classes-1))
            if self.with_axis_keypoints:
                # for param in self.dense_global.parameters():
                #     param.requires_grad = False
                self.orientation_keypoint_head = Vote_Kpoints_head(self.global_feature_dim, keypoint_layers)

                self.orientation_keypoint_predictor = Vote_Kpoints_Predictor(self.keypoint_dim_reduced, 6*(num_classes-1))

            if self.regress_axis:
                self.axis_head = Vote_Kpoints_head(self.global_feature_dim, keypoint_layers)
                self.axis_predictor = Vote_Kpoints_Predictor(self.keypoint_dim_reduced, 4 * (num_classes - 1))

            if self.estimate_norm_vector:
                self.norm_vector_head = Vote_Kpoints_head(1792, keypoint_layers)
    ...
    def forward(self, features, proposals, image_shapes, targets=None):
        ...
        if self.input_mode == config.INPUT_RGBD:
            ...
            shared_features = self.dense_global(shared_features_rgb)  # shared_features_rgb actually has 4-channel RGBD input
            ...
            if self.regress_axis:
                ...
                if self.training:
                    gt_3d_keypoints = [t["axis_keypoints"] for t in targets]
                    ori_depth = [t["ori_image_depth"] for t in targets]
                    # norm_scales = [t["norm_scales"] for t in targets]
        ...
        # if self.input_mode == config.INPUT_RGBD:  # used for 2 separate backbones for RGB and depth
        #     shared_features = self.shared_roi_pool_depth(features, box_proposals, image_shapes)
        #
        #     shared_features = self.dense_global(shared_features)
        #
        #     keypoint_features = self.vote_keypoint_head(shared_features)
        #     keypoint_offsets = self.vote_keypoint_predictor(keypoint_features)
        #
        #     loss_keypoint = {}
        #     if self.training:
        #         gt_vote_offsets = [t["vote_offsets"] for t in targets]  # TODO: check
        #         loss_keypoint = vote_keypoint_loss(keypoint_offsets, box_proposals, gt_vote_offsets, pos_matched_idxs)
        #         loss_keypoint = dict(loss_keypoint=loss_keypoint)
        #     # else:
        #     #     keypoints_probs, kp_scores = vote_keypoint_inference(keypoint_offsets, box_proposals)
...
def paf_loss_updated(paf_logits, gt_masks, mask_matched_idxs, gt_pafs, gt_labels):
    ...
    # loss = torch.mean(torch.abs(paf_logits - target))
    return loss
...
def gt_voters(proposal_height, depth_image, bbox, camera_matrix):
    ...
    # vote_offsets = keypoints_3d.view(bs, 3, 1, 1).repeat(1, 1, proposal_height, proposal_height) - torch.stack((x, y, depth_value), dim=1)
...
def vote_keypoint_loss(keypoint_offsets, proposals, ori_depths, gt_keypoints, keypoint_matched_idxs, masks_for_vote, gt_labels):
    ...
    # vote_offset_norm = torch.norm(votes_targets, dim=1)
    keypoint_loss = torch.sum(keypoint_loss, dim=1)
    # keypoint_loss = keypoint_loss/vote_offset_norm
...
def vote_orientation_loss(keypoint_offsets, proposals, ori_depths, gt_keypoints, keypoint_matched_idxs, masks_for_vote, gt_labels):
    ...
    # masks = masks_for_vote.view(N, 1, H, W).repeat(1, Nk, 1, 1)
    keypoint_loss = torch.mean((keypoint_offsets - votes_targets).abs(), dim=2)
    # vote_offset_norm = torch.norm(votes_targets, dim=2)
    # keypoint_loss = keypoint_loss/vote_offset_norm.unsqueeze(2)
...
def vote_axis_loss(keypoint_offsets, proposals, ori_depths, gt_keypoints, keypoint_matched_idxs, masks_for_vote, gt_labels):
    ...
    voter_axis_labels = torch.norm(vote_offset_targets_all, dim=2).argmin(dim=1).float()
    label_loss = F.binary_cross_entropy_with_logits(keypoint_offsets[:, 3], voter_axis_labels)

    gt_axis = torch.cat(gt_axis, dim=0)

    axis_loss = torch.cross((keypoint_offsets[:, :3] - vote_offset_targets_all[:, 1]), gt_axis, dim=1)
...
def vote_norm_axis_loss(keypoint_offsets, proposals, ori_depths, gt_keypoints, keypoint_matched_idxs, masks_for_vote, gt_labels, norm_scales):
    N, _, _, H, W = keypoint_offsets.shape
    assert H == W
    gt_axis = []
    vote_offset_targets = []
    voter_labels = []
    scales = []
    for proposals_per_image, gt_kp_in_image, midx, ori_depth, label, scale in zip(proposals, gt_keypoints, keypoint_matched_idxs,
                                                                    ori_depths, gt_labels, norm_scales):
        # gt_kp_in_image = gt_kp_in_image/scale.unsqueeze(1)
        kp = gt_kp_in_image[midx].view(-1, 2, 3, 1, 1).repeat(1, 1, 1, H, W)
        voter = gt_voters(H, ori_depth, proposals_per_image, config.Unreal_camera_mat)
        # voter = voter / scale[midx].view(-1, 3, 1, 1)
        gt_offset = kp - voter.unsqueeze(1).repeat(1, 2, 1, 1, 1)
        gt_axis.append(F.normalize((kp[:, 1] - kp[:, 0]), dim=1))
        voter_labels.append(label[midx] - 1)
        vote_offset_targets.append(gt_offset)
        scales.append(scale[midx])
    ...
    gt_axis = torch.cat(gt_axis, dim=0)
    scales = torch.cat(scales, dim=0).view(-1, 3, 1, 1)

    norm_voter_vector = (keypoint_offsets[:, :3] - votes_targets[:, 1])/scales
    axis_loss = torch.cross(norm_voter_vector, gt_axis, dim=1)
    axis_loss = torch.norm(axis_loss, dim=1)
    # gt_axis_distance = torch.cross(votes_targets[:, 1], gt_axis, dim=1)
    # gt_axis_distance = torch.norm(gt_axis_distance, dim=1)
    # axis_loss = axis_loss/gt_axis_distance

    norm_loss = torch.abs(torch.sum(keypoint_offsets[:, :3]/scales * gt_axis, dim=1))
    axis_loss = (axis_loss+norm_loss) * masks_for_vote
    non_zero_index = torch.sum(masks_for_vote, (1, 2)).nonzero()
    axis_loss = torch.mean(
        torch.sum(axis_loss, (1, 2))[non_zero_index] / torch.sum(masks_for_vote, (1, 2))[non_zero_index])

    # scale_loss = torch.mean((keypoint_offsets[:, 4:7] - scales).pow(2), dim=1)
    # scale_loss = scale_loss * masks_for_vote
    # scale_loss = torch.mean(torch.sum(scale_loss, (1, 2))[non_zero_index] / torch.sum(scale_loss, (1, 2))[non_zero_index])
    # norm_loss = torch.mean(torch.norm(keypoint_offsets[:, :3],dim=1))
    # norm_loss = torch.mean(torch.abs(torch.sum(F.normalize(keypoint_offsets[:, :3], dim=1)*gt_axis, dim=1)))
    if torch.isnan(axis_loss):
        print(1)
    return axis_loss + label_loss
