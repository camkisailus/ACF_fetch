        ...
        if self.input_mode == config.INPUT_RGBD:
            ...
            if self.with_3d_keypoints:
                ...
                if self.training:
                    ...
                    gt_labels = [t["labels"] for t in targets]
                    camera_mats = [t["CameraMatrix"] for t in targets]
                    loss_keypoint = vote_keypoint_loss(keypoint_offsets, box_proposals, ori_depth, gt_3d_keypoints,
                                                       pos_matched_idxs, masks_for_vote, gt_labels, camera_mats)
...
def vote_keypoint_loss(keypoint_offsets, proposals, ori_depths, gt_keypoints, keypoint_matched_idxs, masks_for_vote, gt_labels, camera_mats):
    N, K, H, W = keypoint_offsets.shape
    assert H == W
    vote_offset_targets = []
    voter_labels = []
    for proposals_per_image, gt_kp_in_image, midx, ori_depth, label, camera_mat in zip(proposals, gt_keypoints, keypoint_matched_idxs,
                                                                    ori_depths, gt_labels, camera_mats):
        kp = gt_kp_in_image[midx]
        voter = gt_voters(H, ori_depth, proposals_per_image, camera_mat)
...
def vote_orientation_loss(keypoint_offsets, proposals, ori_depths, gt_keypoints, keypoint_matched_idxs, masks_for_vote, gt_labels, camera_mats):
...
def vote_axis_loss(keypoint_offsets, proposals, ori_depths, gt_keypoints, keypoint_matched_idxs, masks_for_vote, gt_labels, camera_mats):
